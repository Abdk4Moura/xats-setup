...
export function installClaude(paths) {
  ...
  const want = { type: 'http', url: `${BASE_URL}/mcp` };
  const cur = cj.mcpServers['cross-agent-teams'];
  if (JSON.stringify(cur) === JSON.stringify(want)) log.info('MCP entry already present');
  else { cj.mcpServers['cross-agent-teams'] = want; writeJson(paths.claudeJson, cj); log.ok('MCP entry cross-agent-teams -> ' + want.url); }
