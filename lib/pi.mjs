...
export async function ensurePiLabel(paths, explicitLabel) {
  ...
  if (!label && isInteractive()) {
    ...
    log.info(`no XATS_LABEL — prompting for a stable Pi label (the prefix of pi-<host>-<label>-<session>).`);
  ...
  if (persisted) {
    ...
    log.info(`Pi identities will be pi-${os.hostname().split('.')[0]}-${label}-<session> (label also saved to ~/.xats/label)`);
  } else {
