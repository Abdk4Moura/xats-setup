...
import { log, isDry, writeFile, ensureDir } from './log.mjs';
import { IS_WIN, IS_MAC, IS_LINUX, IS_ROOT, HOME, nodeBin, which, run } from './platform.mjs';
import { PORT, SERVICE_NAME, BASE_URL } from './constants.mjs';

function startNow(entry) {
  if (isDry()) return log.dry(`start daemon now: node ${entry} daemon --port ${PORT}`);
  try {
    const child = spawn(nodeBin(), [entry, 'daemon', '--port', String(PORT)],
      { detached: true, stdio: 'ignore', windowsHide: true });
    child.unref();
    log.ok('daemon started (detached)');
  } catch (e) {
...
// ---------- Linux / systemd ----------
function installLinux(entry) {
  const systemctl = which('systemctl');
  const unit = [
    '[Unit]',
    'Description=cross-agent-teams (xats) daemon — local cross-agent bus on :' + PORT,
    'After=network.target',
    '',
    '[Service]',
    `ExecStart=${nodeBin()} ${entry} daemon --port ${PORT}`,
    'Restart=always',
    'RestartSec=3',
    '',
    `[Install]`,
...
function installWindows(entry) {
  const tr = `\"${nodeBin()}\" \"${entry}\" daemon --port ${PORT}`;
  if (isDry()) {
  ...
  if (r.ok) { log.ok(`Scheduled Task '${SERVICE_NAME}' registered (runs at logon)`); kind = 'schtasks'; }
    ...
    try { const p = startupVbs(entry); log.ok(`Startup launcher installed (hidden at logon): ${p}`); kind = 'startup-vbs'; }
    catch (e) { log.warn(`Startup launcher failed: ${e.message} — daemon runs this session only`); kind = 'none'; }
  }
  startNow(entry); // don't wait for next logon
  log.info('note: crash-restart is best-effort on Windows; autostart relaunches at each logon');
  return { kind };
}

export function installService(lay) {
  log.step('Service: register daemon to run at login/boot');
  let res;
  if (IS_LINUX) res = installLinux(lay.entry);
  else if (IS_MAC) res = installMac(lay.entry);
  else if (IS_WIN) res = installWindows(lay.entry);
  else { log.warn(`unknown platform ${process.platform}; detached start only`); startNow(lay.entry); res = { kind: 'detached' }; }
  log.info(`health check after start: curl ${BASE_URL}/health`);
  return res;
}
